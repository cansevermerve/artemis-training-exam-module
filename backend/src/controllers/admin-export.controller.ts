...
import { generateTableReportPdf } from "../pdf/reportPdfService.js";
...
export async function exportParticipantsPdf(request: Request, response: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const buffer = await generateTableReportPdf({
      ...
      headers: ["No", "Ad Soyad", "Görev", "Departman", "Durum", "Puan"],
      rows: participantRows(training),
    });
...
export async function exportResultsPdf(request: Request, response: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const buffer = await generateTableReportPdf({
      title: `${training.title} - Sınav Sonuçları`,
      headers: ["No", "Ad Soyad", "Deneme", "Sonuç", "Puan", "Tarih"],
      rows,
    });
